...
In the admin UI → **Collections** → **New collection** → name it `technicians`.
...
Set all 5 API rules to `@request.auth.id != ""`.

Then add one record per team member (just their name). This is what populates the Technician dropdown in the app — add or remove entries here as your team changes.
...
> `@request.auth.id != ""` = authenticated users only (returns 401 for unauthenticated, redirects to login).
> Do not leave any rule set to Locked.

Once set, any request without a valid `Authorization` token gets a 401 and the app redirects to the login screen automatically.
...
`--https=:443` with Let's Encrypt if the VM is publicly reachable.
